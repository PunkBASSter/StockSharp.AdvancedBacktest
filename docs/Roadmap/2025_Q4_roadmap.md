...
БАГ Зигзага: сейчас мы игнорируем бары, на которых одновременно локальный максимум и минимум. Выход: переписать чтобы с 2 буферами работало: peaks/troughts separately (риски - чартовая либа умеет ли строить вертикальные линии? умеет ли строить зигзаг по двум буферам? - нужно 2 буфера, чтобы были вертикальные линии)

Проблемы дебага/репортинга:

Использование SQLite от AI-дебага для обычного дебага и для репортинга?
Сделать хранение 5 последних бэктестов
Рисовать связь защитных ордеров с открывающим.
Монтировать каталог, где лежит SQLite база в RAM - можно ли на винде?

В стратегии может сделать чтобы логика на каждой 5-минутной вспомогательной свече выполнялась? Щас в дебаге м-минутные свечи (т.е. промежутки) оттображаются среди часовых, елси на этой свече что-то произошло.
...
Попробовать брать последний минимум как стоп, не только предшествующий цене открытия.
TP с множителем от СЛ
