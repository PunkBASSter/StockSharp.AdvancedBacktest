...
public class DebugEventBufferTests
{
    ...
    public async Task Flush_OccursAfterSpecifiedInterval()
    {
        ...
        // Act - Uses default candle context (backward compatibility)
    ...
    #region Candle Boundary Tests

    [Fact]
    public void SetCurrentCandle_TagsSubsequentEvents()
    {
        // Arrange
        using var buffer = new DebugEventBuffer(10000);
        var candle1Time = DateTimeOffset.Parse("2024-01-01T10:00:00Z");
        var candle2Time = DateTimeOffset.Parse("2024-01-01T11:00:00Z");

        Dictionary<string, List<object>>? flushedEvents = null;
        buffer.OnFlush += events => flushedEvents = events;

        // Act - Set candle 1, add events
        buffer.SetCurrentCandle(candle1Time);
        buffer.Add("candle", new CandleDataPoint { Time = candle1Time.ToUnixTimeMilliseconds() });
        buffer.Add("trade", new TradeDataPoint { Time = candle1Time.ToUnixTimeMilliseconds() });

        // Move to candle 2
        buffer.SetCurrentCandle(candle2Time);
        buffer.Add("candle", new CandleDataPoint { Time = candle2Time.ToUnixTimeMilliseconds() });

        // Flush - should only flush candle 1 (candle 2 is current)
        buffer.Flush();
        Thread.Sleep(50);

        // Assert - Candle 1 events flushed, candle 2 not yet
        Assert.NotNull(flushedEvents);
        Assert.Equal(2, flushedEvents.Count); // candle and trade from candle 1
        Assert.Single(flushedEvents["candle"]);
        Assert.Single(flushedEvents["trade"]);
    }

    [Fact]
    public void Flush_OnlyFlushesCompletedCandles()
    {
        // Arrange
        using var buffer = new DebugEventBuffer(10000);
        var candle1Time = DateTimeOffset.Parse("2024-01-01T10:00:00Z");
        var candle2Time = DateTimeOffset.Parse("2024-01-01T11:00:00Z");
        var candle3Time = DateTimeOffset.Parse("2024-01-01T12:00:00Z");
        ...
        // Act
        buffer.SetCurrentCandle(candle1Time);
        buffer.Add("trade", new TradeDataPoint { Time = candle1Time.ToUnixTimeMilliseconds(), Volume = 100 });

        buffer.SetCurrentCandle(candle2Time);
        buffer.Add("trade", new TradeDataPoint { Time = candle2Time.ToUnixTimeMilliseconds(), Volume = 200 });

        buffer.SetCurrentCandle(candle3Time);
        buffer.Add("trade", new TradeDataPoint { Time = candle3Time.ToUnixTimeMilliseconds(), Volume = 300 });

        // Flush - should flush candle 1 and 2, but not 3 (current)
        buffer.Flush();
        Thread.Sleep(50);

        // Assert
        Assert.NotNull(flushedEvents);
        Assert.Single(flushedEvents); // Only "trade" event type
        Assert.Equal(2, flushedEvents["trade"].Count); // 2 trades from candles 1 and 2
    }

    [Fact]
    public void FlushSynchronously_OnlyFlushesCompletedCandles()
    {
        // Arrange
        using var buffer = new DebugEventBuffer(10000);
        var candle1Time = DateTimeOffset.Parse("2024-01-01T10:00:00Z");
        ...
        // Assert - Only candle 1 events should be flushed
        Assert.NotNull(flushedEvents);
    ...
    [Fact]
    public void Dispose_FlushesAllCandlesIncludingCurrent()
    {
        // Arrange
        var buffer = new DebugEventBuffer(10000);
        var candle1Time = DateTimeOffset.Parse("2024-01-01T10:00:00Z");
        ...
        // Dispose - should flush ALL candles including current
        buffer.Dispose();

        // Assert
        Assert.NotNull(flushedEvents);
        Assert.Single(flushedEvents); // Only "trade" event type
        Assert.Equal(2, flushedEvents["trade"].Count); // Both trades
    }

    [Fact]
    public void CandleBoundary_EventsOrderedByCandle()
    {
        // Arrange
        using var buffer = new DebugEventBuffer(10000);
        var candle1Time = DateTimeOffset.Parse("2024-01-01T10:00:00Z");
        ...
        var allFlushedTrades = new List<TradeDataPoint>();
        buffer.OnFlush += events =>
        {
            if (events.ContainsKey("trade"))
            {
                allFlushedTrades.AddRange(events["trade"].Cast<TradeDataPoint>());
            }
        };

        // Act - Add events in order, flush after each candle change
        buffer.SetCurrentCandle(candle1Time);
        buffer.Add("trade", new TradeDataPoint { Time = candle1Time.ToUnixTimeMilliseconds(), Volume = 100 });
        ...
        // Assert - Events should be ordered by candle time
        Assert.Equal(2, allFlushedTrades.Count);
        Assert.Equal(100, allFlushedTrades[0].Volume);
        Assert.Equal(200, allFlushedTrades[1].Volume);
    }

    [Fact]
    public void CandleBoundary_MultipleEventTypesPerCandle()
    {
        // Arrange
        using var buffer = new DebugEventBuffer(10000);
        var candleTime = DateTimeOffset.Parse("2024-01-01T10:00:00Z");

        Dictionary<string, List<object>>? flushedEvents = null;
        buffer.OnFlush += events => flushedEvents = events;

        // Act - Add multiple event types for same candle
        buffer.SetCurrentCandle(candleTime);
        buffer.Add("candle", new CandleDataPoint { Time = candleTime.ToUnixTimeMilliseconds() });
        buffer.Add("trade", new TradeDataPoint { Time = candleTime.ToUnixTimeMilliseconds() });
        buffer.Add("trade", new TradeDataPoint { Time = candleTime.ToUnixTimeMilliseconds() });
        buffer.Add("indicator", new IndicatorDataPoint { Time = candleTime.ToUnixTimeMilliseconds() });

        // Move to next candle to trigger flush
        buffer.SetCurrentCandle(candleTime.AddHours(1));
        buffer.Flush();
        Thread.Sleep(50);

        // Assert - All event types from first candle should be flushed together
        Assert.NotNull(flushedEvents);
        Assert.Equal(3, flushedEvents.Count); // candle, trade, indicator
        Assert.Single(flushedEvents["candle"]);
        Assert.Equal(2, flushedEvents["trade"].Count);
        Assert.Single(flushedEvents["indicator"]);
    }
