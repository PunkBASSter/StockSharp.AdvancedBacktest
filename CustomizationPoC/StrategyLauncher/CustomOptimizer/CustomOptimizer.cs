...
public class CustomOptimizer<TStrategy> : ICustomOptimizer<TStrategy> where TStrategy : CustomStrategyBase, new()
{
	private ManualResetEventSlim _optimizationCompleteEvent = new(false);
	private OptimizationConfig _config;
	...
	public Dictionary<string, OptimizationResult<TStrategy>> Optimize()
	{
		...
		OptimizationStrategies = optimizationPairs.Select(pair => pair.strategy as TStrategy).ToList();
		...
		var validationStrategies = validationPairs.ToDictionary(
			pair => (pair.strategy as TStrategy).Hash,
			pair => pair.strategy as TStrategy);
